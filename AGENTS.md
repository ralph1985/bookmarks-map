...
- `public/` stores static assets (favicons, manifest, default bookmark samples). Keep large fixture files under `public/data/`.
...
- Start local development using `npm run dev`; the app reads a bookmark file via in-browser `FileReader`.
...
## Testing Guidelines
- Unit tests rely on Vitest with Testing Library; add one test case per branch in parsing helpers, using fixture files under `tests/fixtures/`.
